    ...
    <div style={{
      ...
      <div className="glass-panel" style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px', borderBottom: '1px solid rgba(255, 255, 255, 0.1)', paddingBottom: '16px' }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <div>
                <h2 style={{ fontSize: '1.5rem', fontWeight: 800, color: '#1a2638' }}>
                  {block.block_id} — Operational Plan Details
                </h2>
                <div style={{ fontSize: '0.85rem', color: '#71829d', marginTop: '2px' }}>
        ...
        <div style={{ marginBottom: '24px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '14px', background: '#f1f5f9', padding: '16px', borderRadius: '10px' }}>
            <div>
              <span style={{ fontSize: '0.75rem', color: '#71829d', fontWeight: 600 }}>SCHEDULED DATE</span>
              <div style={{ fontSize: '1.05rem', fontWeight: 800, color: '#1a2638', marginTop: '2px' }}>{block.date}</div>
            </div>
            <div>
              <span style={{ fontSize: '0.75rem', color: '#71829d', fontWeight: 600 }}>START TIME</span>
              <div style={{ fontSize: '1.05rem', fontWeight: 800, color: '#10B981', marginTop: '2px' }}>{block.block_start || '00:00'}</div>
            </div>
            <div>
              <span style={{ fontSize: '0.75rem', color: '#71829d', fontWeight: 600 }}>END TIME</span>
              <div style={{ fontSize: '1.05rem', fontWeight: 800, color: '#EF4444', marginTop: '2px' }}>{block.block_end || '03:00'}</div>
            </div>
            <div>
              <span style={{ fontSize: '0.75rem', color: '#71829d', fontWeight: 600 }}>DURATION</span>
        ...
        <div style={{ marginBottom: '24px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '14px', background: '#f1f5f9', padding: '16px', borderRadius: '10px' }}>
            <div>
              <span style={{ fontSize: '0.75rem', color: '#71829d', fontWeight: 600 }}>CORRIDOR</span>
              <div style={{ fontSize: '0.95rem', fontWeight: 700, color: '#1a2638', marginTop: '2px' }}>Corridor {block.corridor}</div>
            </div>
            <div>
              <span style={{ fontSize: '0.75rem', color: '#71829d', fontWeight: 600 }}>SECTION</span>
              <div style={{ fontSize: '0.95rem', fontWeight: 700, color: '#1a2638', marginTop: '2px' }}>{block.section || 'C1-S1'}</div>
            </div>
            <div>
              <span style={{ fontSize: '0.75rem', color: '#71829d', fontWeight: 600 }}>WORK AREA</span>
              <div style={{ fontSize: '0.95rem', fontWeight: 700, color: '#3B82F6', marginTop: '2px' }}>{block.work_area}</div>
            </div>
            <div>
              <span style={{ fontSize: '0.75rem', color: '#71829d', fontWeight: 600 }}>TASKS INCLUDED</span>
              <div style={{ fontSize: '0.95rem', fontWeight: 700, color: '#1a2638', marginTop: '2px' }}>{block.allocated_tasks?.join(', ')}</div>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '16px', marginBottom: '24px' }}>
          {/* Workforce */}
          <div style={{ background: '#f1f5f9', padding: '18px', borderRadius: '10px', border: '1px solid rgba(52, 211, 153, 0.3)' }}>
            ...
            <div style={{ fontSize: '0.85rem', color: '#71829d' }}>
          ...
          {/* Equipment */}
          <div style={{ background: '#f1f5f9', padding: '18px', borderRadius: '10px', border: '1px solid rgba(251, 191, 36, 0.3)' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
        ...
        <div style={{ background: '#e9eff7', padding: '20px', borderRadius: '12px', marginBottom: '24px', border: '1px solid rgba(56, 189, 248, 0.4)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '12px', fontSize: '0.85rem', color: '#71829d', marginBottom: '16px' }}>
            <div>Request ID: <strong style={{ color: '#1a2638' }}>{block.request_details_in_group?.[0]?.request_id || block.requests_in_group?.join(', ') || 'REQ001'}</strong></div>
            <div>Asset ID: <strong style={{ color: '#1a2638' }}>{block.request_details_in_group?.[0]?.asset_id || block.work_area || 'TRK001'}</strong></div>
            <div>Defect Type: <strong style={{ color: '#1a2638' }}>{block.request_details_in_group?.[0]?.defect_type || 'Track Defect'}</strong></div>
            ...
            <div>Required Duration: <strong style={{ color: '#1a2638' }}>{block.allocated_duration_minutes} min</strong></div>
            <div>Allocated Duration: <strong style={{ color: '#10B981' }}>{block.allocated_duration_minutes} min</strong></div>
            <div>Workers Required: <strong style={{ color: '#1a2638' }}>{block.workers_required}</strong></div>
            <div>Workers Assigned: <strong style={{ color: '#10B981' }}>{block.assigned_worker_details?.length || block.workers_required}</strong></div>
            <div>Equipment Required: <strong style={{ color: '#1a2638' }}>{block.assigned_equipment?.join(', ') || 'Track Machine'}</strong></div>
            ...
            <div>Selected Block: <strong style={{ color: '#1a2638' }}>{block.block_id}</strong></div>
          ...
          <div style={{ background: '#f1f5f9', padding: '12px 16px', borderRadius: '8px', borderLeft: '4px solid #10b981' }}>
            ...
            <p style={{ fontSize: '0.88rem', color: '#1a2638', margin: 0, lineHeight: 1.5 }}>
