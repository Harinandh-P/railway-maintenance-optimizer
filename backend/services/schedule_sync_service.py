...
def sync_schedule_statuses(current_time_override: Optional[datetime] = None) -> Dict[str, Any]:
    ...
    # Save updated block lifecycle statuses back to final_block_plan.json
    try:
        with open(AppConfig.FINAL_BLOCK_PLAN, "w") as f:
            json.dump(plan_data, f, indent=2)
    except Exception as e:
        print(f"[SYNC SERVICE WARNING] Failed to write updated final_block_plan.json: {e}")
...
def sync_pipeline_execution_results(selected_request_ids: Set[str], current_time_override: Optional[datetime] = None) -> Dict[str, Any]:
    """
    Post-Pipeline Request Status Synchronization.
    - selected_request_ids: Set of request IDs selected for the pipeline run.
    - Allocated in Phase 3 final_block_plan -> SCHEDULED (or COMPLETED if end_time <= now).
    - NOT allocated in Phase 3 final_block_plan -> PENDING.
    - Unselected requests -> Database status remains 100% UNTOUCHED.
    - Updates final_block_plan.json block lifecycle_status.
    """
    now = current_time_override if current_time_override else datetime.now()
    ...
    for block in blocks:
        ...
        # Collect associated request IDs for block from all 3 possible fields
        block_req_ids: Set[str] = set()
        for t in (block.get("allocated_tasks") or []):
            if t:
                block_req_ids.add(str(t).strip())
        for t in (block.get("requests_in_group") or []):
            if t:
                block_req_ids.add(str(t).strip())
        for r in block.get("request_details_in_group", []):
            if isinstance(r, dict) and r.get("request_id"):
    ...
    selected_ids_clean = {str(s).strip() for s in selected_request_ids if s}

    for r in existing_requests:
        rid = str(r.get("request_id", "")).strip()
        curr_status = str(r.get("status", "PENDING")).upper().strip()

        # Terminal statuses (REJECTED, CANCELLED) remain unchanged
        if curr_status in ("REJECTED", "CANCELLED"):
            updated_requests.append(r)
            continue

        if rid in selected_ids_clean:
            if rid in expired_completed_req_ids:
                target_status = "COMPLETED"
            elif rid in active_scheduled_req_ids:
                target_status = "SCHEDULED"
            else:
                # Selected, but NOT allocated by Phase 3 -> PENDING
                target_status = "PENDING"
        else:
            # Unselected request: DB status remains 100% UNTOUCHED
            target_status = curr_status
