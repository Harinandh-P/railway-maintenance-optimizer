...
def compare_reasons(current_reason, historical_reason):
    ...
    if current_category == "Unknown" and historical_category == "Unknown":
        return {
            "score": None,
            "match": "Unknown",
    ...
    # Both are known and belong to the same category
    ...
    # Both are known but belong to different categories
    return {
        "score": 0.0,
        "match": "Different",
        "current_category": current_category,
...
def compare_assets(current_asset, historical_asset):
    ...
    # Known different asset IDs
    return {
        "score": 0.0,
        "match": "Different_Asset"
    }
def historical_match_engine(current_request, historical_record):
    ...
    # No usable comparison information
    if not scores:
        return {
            "score": None,
            "match": "No_Evidence",
    ...
    # Normalize based only on available evidence
    final_score = sum(weighted_scores) / sum(scores)
...
def rank_historical_records(current_request, historical_records):
    ...
    for record in historical_records:
        ...
        ranked_records.append({
            ...
            "required_workers": record.get("workers_used"),
            "previous_failure": record.get("previous_failure"),
            "maintenance_frequency": record.get("maintenance_frequency"),
            "previous_priority": record.get("previous_priority"),
            "operational_impact": record.get("operational_impact"),
...
def estimate_duration(ranked_records):
    ...
    for record in ranked_records:
        ...
        # Only use sufficiently relevant historical records
    ...
    return {
        "estimated_duration": round(weighted_duration, 2),
        "method": "Weighted_Historical_Average",
        "records_used": len(durations)
    }
def estimate_workers(ranked_records):
    ...
    for record in ranked_records:
        ...
        workers_used = record.get("required_workers")

        if workers_used is None:
            continue

        try:
            workers_used = float(workers_used)

        except (ValueError, TypeError):
            continue

        # Ignore invalid worker values
        if workers_used <= 0:
            continue

        workers.append(workers_used)
        weights.append(score)

    # No usable historical worker data
    if not workers:
...
def build_maintenance_requirement(
    ...
    return {
        ...
        "required_duration":
            duration_result.get("estimated_duration"),

        "required_workers":
            worker_result.get("estimated_workers"),
        ...
        "asset_risk_score":
            asset_risk.get("asset_risk_score"),

        "asset_risk_level":
            asset_risk.get("risk_level"),
...
def extract_historical_features(ranked_records, historical_records):
    ...
    relevant_task_ids = {
        ...
        if (
            ...
            and record["score"] >= RELEVANCE_THRESHOLD
        )
    }
    ...
    relevant_records = [
        record
        for record in historical_records
        if record.get("task_id") in relevant_task_ids
    ]
    ...
    failure_values = [
        ...
        if record.get("previous_failure") is not None
    ]

    failure_count = sum(
        1 for value in failure_values
        if value is True
    )

    if failure_values:
        failure_frequency = failure_count / len(failure_values)
    else:
        failure_frequency = None

    # Actual maintenance duration
    duration_values = [
    ...
    priority_values = [
        ...
        for record in relevant_records
        if isinstance(record.get("previous_priority"), (int, float))
    ]
    ...
    frequency_values = [
        ...
        for record in relevant_records
        if isinstance(record.get("maintenance_frequency"), (int, float))
    ]
    ...
    impact_values = [
        ...
        for record in relevant_records
        if record.get("operational_impact")
    ]
    ...
    return {
        ...
        if failure_frequency is not None else None,
        "average_duration": round(average_duration, 2)
        if average_duration is not None else None,
        "average_previous_priority": round(
            average_previous_priority, 2
        )
        if average_previous_priority is not None else None,
        "average_maintenance_frequency": round(
            average_maintenance_frequency, 2
        )
        if average_maintenance_frequency is not None else None,
        "operational_impact": highest_impact
    }
...
def calculate_asset_risk(asset_data):
    ...
    traffic_score = traffic_map.get(
        traffic_importance,
        None
    )

    # Failure history score
    if isinstance(failure_history, (int, float)):
    ...
    if isinstance(trains_per_day, (int, float)):
    ...
    # Equal weighting for initial prototype
    asset_risk_score = (
        sum(valid_scores) /
        len(valid_scores)
    )

    # Convert numerical risk into a category
    ...
    return {
        ...
        "failure_history_score": failure_score,

        "traffic_importance_score": traffic_score,

        "traffic_volume_score": traffic_volume_score
    }

def calculate_current_request_risk(current_request):
    ...
    if not valid_scores:
        ...
        return {
            "current_request_risk_score": None,
            "risk_level": "Unknown"
        }
    ...
    return {
        ...
        "current_request_risk_score": round(
            current_risk_score,
            3
        ),
...
def calculate_priority_score(
    ...
    current_score = current_request_risk.get(
        "current_request_risk_score"
    )
    ...
    historical_score = historical_features.get(
        "failure_frequency"
    )
    ...
    previous_priority = historical_features.get(
        "average_previous_priority"
    )
    ...
    operational_impact = historical_features.get(
        "operational_impact"
    )
    ...
    operational_score = operational_map.get(
        operational_impact
    )
    ...
    return {
        ...
        "current_risk_contribution": round(
            current_score * CURRENT_WEIGHT,
            3
        ) if current_score is not None else None,

        "asset_risk_contribution": round(
            asset_score * ASSET_WEIGHT,
            3
        ) if asset_score is not None else None,

        "historical_failure_contribution": round(
            historical_score * HISTORICAL_WEIGHT,
            3
        ) if historical_score is not None else None,

        "operational_impact_contribution": round(
            operational_score * OPERATIONAL_WEIGHT,
            3
        ) if operational_score is not None else None,

        "previous_priority_contribution": round(
            previous_priority_score * PREVIOUS_PRIORITY_WEIGHT,
            3
        ) if previous_priority_score is not None else None,
...
def generate_priority_explanation(
    ...
    if operational_impact is not None:
        ...
        # If operational impact is already numeric
        if isinstance(operational_impact, (int, float)):
        ...
        # If operational impact is text
        else:

            impact = str(operational_impact).lower()
...
def prepare_real_data():
    ...
    for _, row in history_df.iterrows():
        ...
        historical_records.append({
            ...
            "defect": row.get("defect_type"),

            "reason": row.get("defect_reason"),

            "previous_failure": previous_failure,

            "actual_duration": row.get(
                "actual_duration_hours"
            ),

            "workers_used": row.get(
                "workers_used"
            ),

            "previous_priority": row.get(
                "previous_priority"
            ),

           "operational_impact": row.get(
...
"maintenance_frequency": (
    (
        pd.to_datetime(
            row.get("next_scheduled_maintenance")
        )
        -
        pd.to_datetime(
    ...
    if pd.notna(row.get("maintenance_date"))
    and pd.notna(row.get("next_scheduled_maintenance"))
    else None
)
        })
    ...
    for _, row in assets_df.iterrows():
        ...
        asset_records[
            ...
            "asset_condition": row.get(
                "asset_condition"
            ),

            "failure_history": row.get(
                "total_failure_count"
            ),

            "traffic_importance": row.get(
                "traffic_importance"
            ),

            "trains_per_day": row.get(
    ...
    for _, row in requests_df.iterrows():
        ...
        current_requests.append({
            ...
            "required_duration": row.get(
                "required_duration_hours"
            ),

            "required_workers": row.get(
                "required_workers"
            ),

            "required_equipment": row.get(
                "required_equipment"
            )
        })
...
if __name__ == "__main__":
    ...
    for current_request in current_requests:
        ...
        for feature, value in asset_risk.items():
        ...
        for feature, value in priority_result.items():
...
maintenance_requirement = build_maintenance_requirement(
    current_request,
    current_request_risk,
    priority_result,
    duration_result,
...
for feature, value in maintenance_requirement.items():
